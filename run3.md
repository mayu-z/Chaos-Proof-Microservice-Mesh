...
| Traffic controller | Go + controller-runtime | Owns Envoy weight patching, drain logic, recovery ramp |
| Incident controller | Go + controller-runtime | Owns PostgreSQL incident records and Slack post-mortem |
| Coordination controller | Go + controller-runtime | Owns HPA freeze and ArgoCD sync pausing |
...
│   └── argocd/
│       └── app-of-apps.yaml
├── controllers/                # Go controllers (split by responsibility)
│   ├── traffic/                # Envoy weight patching, drain, recovery ramp
│   │   ├── main.go
│   │   ├── controller.go
│   │   └── Dockerfile
│   ├── incident/               # PostgreSQL incident records, Slack post-mortem
│   │   ├── main.go
│   │   ├── controller.go
│   │   └── Dockerfile
│   └── coordination/           # HPA freeze, ArgoCD sync pausing
│       ├── main.go
│       ├── controller.go
│       └── Dockerfile
...
The Go controller runs as multiple replicas with leader election via `controller-runtime`. Only the replica holding the leader lease acts — the others stand by. If the active replica crashes mid-incident, a standby acquires the lease within seconds and resumes from the last written state in PostgreSQL. No incident is lost, no routing decision is left in an undefined state.

```
controller replica 1  ──► holds leader lease ──► acts
controller replica 2  ──► standby
controller replica 3  ──► standby

if replica 1 crashes:
  replica 2 acquires lease in < 10 seconds
  reads last incident state from PostgreSQL
  resumes reconciliation from where replica 1 stopped
```

The three controllers have split responsibilities to keep each one focused and testable:

- **Traffic controller** — owns Envoy weight patching, drain logic, and recovery ramp
- **Incident controller** — owns the PostgreSQL incident record and Slack post-mortem
- **Coordination controller** — owns HPA freezing and ArgoCD sync pausing

They communicate via a shared CRD (`RegionalIncident`). The traffic controller writes to it, the other two watch it and react. No direct calls between controllers.

The traffic controller runs a continuous reconciliation loop:
...
every 2 minutes:
  ...
  if burn rate > 2x threshold in a region:
    ...
    check headroom on receiving regions first:
      if receiving regions are below 70% RPS capacity:
        drain degraded region to 0% in two steps (50% → 0%, 30s apart)
        receiving regions absorb load gradually while HPA scales them up
        
      if receiving regions are above 70% RPS capacity:
        do NOT fully evacuate — drain to minimum viable (20%)
        activate load shedding on receiving regions:
          rate limit low-priority traffic (feed refresh, recommendations)
          reject non-critical background requests
          preserve writes and timeline reads
        fire critical alert — human awareness required
        this is a multi-region stress event, not a single region incident
        
    freeze HPA scaling for degraded region (annotation on HPA object)
    pause ArgoCD sync for degraded namespace
    open incident record in PostgreSQL
    
  if region was draining and ALL of the following are healthy:
    p99 burn rate back within SLO
    error rate < 0.1%
    Kafka consumer lag < 30 seconds behind
    memory utilization < 80%
    connection pool saturation < 70%
    replication lag < threshold
    
    only then: ramp traffic back: 10% → 30% → 60% → 100%
    run health check between each step
    unfreeze HPA and ArgoCD sync
    close incident record with MTTR
    post Slack summary
    
    if any signal is still unhealthy: hold, wait for next cycle
...
The 2-minute reconciliation loop is a consistency check, not the primary detection mechanism. Fast detection runs through Prometheus alerting rules evaluated every 15 seconds — when burn rate crosses the threshold, Alertmanager fires a webhook directly to the traffic controller. The controller reacts in seconds, not minutes.

The 2-minute loop then reconciles state, handles any edge cases the alert missed, and ensures the system is consistent with what Prometheus actually shows. A 2-pod failure handled by local Envoy circuit breaking should not trigger the alert at all — the alert threshold is calibrated above the noise floor of normal pod churn.
...
## Architectural Decisions and Mitigations

**Controller SPOF** — The traffic controller runs as multiple replicas with leader election via `controller-runtime`. A replica crash during an active incident is handled by lease takeover within 10 seconds, with state recovered from PostgreSQL.

**Cascading regional failure** — Before evacuating a region the controller checks headroom on receiving regions. If they are above 70% RPS capacity, full evacuation is blocked and load shedding activates instead. Full evacuation only happens when receiving regions have confirmed capacity.

**Slow detection** — Primary detection runs through Prometheus alerting rules on a 15-second evaluation interval with Alertmanager webhooks to the controller. The 2-minute reconciliation loop is a consistency backstop, not the detection path.

**Recovery signal quality** — Recovery requires six signals to be healthy simultaneously: p99 burn rate, error rate, Kafka consumer lag, memory utilization, connection pool saturation, and replication lag. p99 alone is not sufficient to declare a region recovered.

**Controller complexity** — Three controllers with split responsibilities communicate via a shared `RegionalIncident` CRD. Each controller is independently testable and has a single clear ownership boundary.

**CRD dependency during control plane stress** — If a chaos event degrades the Kubernetes API server or etcd, CRD writes from the traffic controller may not reach the coordination controller in time. To avoid this, the traffic controller performs time-critical actions — freezing HPA and patching Envoy weights — via direct Kubernetes API calls at the moment of detection, not via CRD propagation. The `RegionalIncident` CRD is written immediately after as an audit trail. The coordination controller reconciles against it to ensure freeze state is maintained, but it is not the actor that initiates the freeze. Critical actions happen regardless of CRD delivery latency.

**Load shedding flap cycle** — Load shedding suppresses symptoms: p99 drops, error rate drops, metrics look healthy. Without a guard the controller would deactivate load shedding, traffic rushes back in, metrics spike, load shedding reactivates — an automated flap cycle. The fix is a minimum hold duration: once load shedding activates it cannot be deactivated for 10 minutes regardless of what metrics show. After 10 minutes the controller evaluates two conditions before releasing: receiving region RPS must be below 60% capacity AND the degraded region must be fully recovered. If either fails, the hold extends by 5 minutes. Load shedding releases gradually over 5 minutes, not instantly.

**Passive health checks miss real load failure modes** — A `/healthz` ping at 10% traffic does not tell you what happens at 60% or 100%. PostgreSQL lock contention, file descriptor exhaustion, and gRPC thread pool saturation do not surface under low load. Health check steps now include synthetic load injection: before stepping from 10% to 30%, the controller injects synthetic traffic at 35% equivalent load for 30 seconds and measures real p99, error rate, and connection pool saturation under that load. Only if all signals hold does it proceed to the next step. The 60-second window between steps includes 30 seconds of synthetic load validation, not just passive waiting.

**Kafka rate limiting creates a false recovery signal** — Rate-limited consumers protect a recovering region from thundering herd but create artificial lag that never clears while real user traffic is zero. The controller waits for lag to drop below 30 seconds, it drops because there is no new traffic, controller restores traffic, new traffic generates new lag, controller evacuates again. The fix is to check consumer processing rate relative to incoming event rate rather than raw lag: the consumer must be processing at a rate equal to or greater than the current incoming event rate plus 10% headroom, proving it can handle live production load and still make progress on backlog. Additionally the consumer rate limit tracks the traffic ramp — at 10% traffic the consumer runs at 10% of normal rate plus backlog clearance headroom, at 30% it runs at 30%, and so on. The rate limit is not a fixed cap.
...
- Three-controller architecture with leader election — traffic, incident, and coordination controllers communicating via shared CRD
- Critical actions (HPA freeze, Envoy patch) executed via direct API calls, not CRD propagation — CRD is audit trail only, not control signal
- Fast incident detection via Prometheus alerting rules + Alertmanager webhook (15-second evaluation) with 2-minute reconciliation as consistency backstop
- Cascading failure prevention — headroom check on receiving regions before evacuation, load shedding with 10-minute minimum hold and gradual release
- Load shedding flap prevention — minimum hold duration with dual release condition: receiving region capacity AND degraded region recovery both confirmed
- Multi-signal recovery validation — six signals must be healthy before traffic restoration, not just p99
- Synthetic load injection during recovery ramp — real p99 measured under 35% equivalent load before each step, not passive health pings
- Kafka recovery signal based on consumer processing rate vs incoming event rate, not raw lag — rate limit tracks traffic ramp proportionally
